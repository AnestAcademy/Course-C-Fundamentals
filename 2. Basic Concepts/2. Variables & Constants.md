...
Đối với kiểu số thập phân (`float` và `double`) cần lưu ý khi khai báo và gán giá trị cho biết, trong lập trình chúng ta sẽ sử dụng dấu chấm `.` thay vì dấu phẩy `,` - mặc dù khi nói hay viết chúng ta vẫn hay sử dụng cụm từ `dấu phẩy động` để ám chỉ các số thập phân.
```c
#include <stdio.h>
...
Nếu bạn muốn xác định một biến có giá trị **không thể thay đổi**, bạn có thể sử dụng 2 cách sau:
...
- Quy tắc đặt tên biến hằng số hơi khác với quy tắc đặt tên biến thông thường đó là sẽ viết hoa tất cả ký tự tên biến, nếu tên biến hằng số có 2 từ, sẽ được ngăn cách bằng dấu gạch dưới `_`. Ví dụ: `PI`, `NEW_LINE`...
