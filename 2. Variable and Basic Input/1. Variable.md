...
`kiểu dữ liệu` `tên biến số` = `giá trị;`
...
Bây giờ, chúng ta sẽ xem làm sao để hiển thị một giá trị của biến số cũng với hàm này. Chúng ta cũng sẽ sử dụng `printf()` với phương pháp cũ, nhưng thêm vào một định dạng đặc biệt tại vị trí mà chúng ta muốn hiển thị giá trị của biến đó. 
...
Chúng ta đã chỉ ra vị trí cần hiển thị một số, nhưng chúng ta vẫn chưa nói là hiển thị số nào. Vì thể chúng ta cần chỉ cho hàm `printf()` biết phải hiển thị biến số nào. 
