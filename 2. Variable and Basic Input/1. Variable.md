...
Ví dụ một định dạng đặc biệt đó là `%d` - kí tự này cho phép chúng ta hiển thị giá trị của biến `int` trong hàm `printf()`. 

Vì vậy ứng với mỗi kiểu dữ liệu, chúng ta sẽ có ký tự đặc biệt của kiểu dữ liệu đó như sau:
