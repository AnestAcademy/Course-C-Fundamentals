...
`return` - đây là từ khoá dùng để kết thúc một hàm xử lý, khi gặp từ khoá `return` thì chương trình sẽ không chạy tiếp các dòng mã bên dưới từ `return` nữa. Trong ví dụ trên, khi nhập `n < 2` thì sẽ chạy vào câu lệnh `if` và thực hiện lệnh `return 0;`
